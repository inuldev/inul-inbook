...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        console.log("Token cookie check after login:", hasToken);
        console.log("All cookies:", document.cookie);

        if (hasToken) {
          console.log("Token cookie successfully set");
          router.push("/");
        } else {
          console.warn(
            "Token cookie not set after login - retrying navigation"
          );
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-12 bg-background text-foreground">
      {redirecting && <LoadingOverlay message="Logging you in..." />}
